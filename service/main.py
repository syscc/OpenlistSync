"""
OpenListSync Android 服务入口（运行在 :pythonservice 进程）。
...
#    路径为 /storage/emulated/0/Android/data/<包名>/files/openlistsync_debug.log）
# 2. 上面路径的硬编码形式（包名 com.github.openlistsync），jnius 不可用时兜底
...
try:
    ...
    if _d is not None:
        _log_paths.append(os.path.join(str(_d.getAbsolutePath()), 'openlistsync_debug.log'))
except Exception:
    pass
_log_paths.append('/storage/emulated/0/Android/data/com.github.openlistsync/files/openlistsync_debug.log')
...
# 安装日志收集
# ======================================================================
_file_log('INFO', '=== OpenListSync 服务进程启动 ===')
...
from common.httpApp import make_app
...
<title>OpenListSync Android</title>
<style>
...
<body>
<nav id="tabs" role="tablist" aria-label="OpenListSync 页面">
...
<section id="viewLogs" class="view active" aria-labelledby="tabLogs" role="tabpanel">
  <div id="bar">
    <span id="status"></span>
    <span class="title">OpenListSync 运行日志</span>
    <button type="button" id="btnClear">清空</button>
  </div>
  <div id="log"></div>
</section>
<section id="viewWeb" class="view" aria-labelledby="tabWeb" role="tabpanel" hidden>
  <iframe id="businessFrame" title="OpenListSync 网页" data-src="http://127.0.0.1:__BUSINESS_PORT__/"></iframe>
</section>
<script>
var lastSeq = 0;
...
class LogIndexHandler(RequestHandler):
    def get(self):
        ...
        self.write(_LOG_PAGE_HTML.replace('__BUSINESS_PORT__', str(BUSINESS_PORT)))
...
async def main():
    ...
    business_app = make_app(server_cfg, os.path.join(_app_dir, 'front'))
    # 业务端口监听 0.0.0.0，允许外部设备访问；服务进程不会随 Activity 冻结。
