...
const ensureOpenlistList = async () => {
  if (openlistList.value.length > 0) return;
  const res = await openlistGet();
...
<template>
  <div class="mobile-job-page">
    ...
    >
      <el-form v-if="editData" ref="jobRule" :model="editData" :rules="addRule" label-position="top">
        <section class="editor-section">
          ...
          <el-form-item :label="$t('home.engine')" prop="openlistId">
            <el-select v-model="editData.openlistId" :placeholder="$t('home.requiredEngine')" :no-data-text="$t('home.noEngine')">
              <el-option v-for="item in openlistList" :key="item.id" :label="`${item.url}${item.remark ? ` [${item.remark}]` : ''}`" :value="item.id" />
            </el-select>
          </el-form-item>
        ...
        <section class="editor-section">
          ...
          <el-form-item :label="$t('home.srcPath')" prop="srcPath">
            <div v-if="editData.openlistId == null" class="field-placeholder">{{ $t("home.selectEngineFirst") }}</div>
    ...
    <pathSelect v-if="editData" ref="pathSelectRef" :openlist-id="editData.openlistId" @submit="submitPath" />
  </div>
</template>
