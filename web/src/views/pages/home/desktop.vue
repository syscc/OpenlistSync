...
const editJobShow = (row) => {
  ...
  if (openlistList.value.length === 0) {
    getOpenlistList();
  }
  excludeTmp.value = "";
...
<template>
  <div class="home table-page">
    ...
    >
      <div class="elform-box">
        <el-form :model="editData" :rules="addRule" ref="jobRule" v-if="editShow" label-width="150px">
          ...
          <el-form-item prop="openlistId" :label="$t('home.engine')">
            <el-select v-model="editData.openlistId" :placeholder="$t('home.requiredEngine')" class="label-width" :no-data-text="$t('home.noEngine')">
          ...
          <el-form-item prop="srcPath" :label="$t('home.srcPath')">
            <div v-if="editData.openlistId == null" class="label-width">{{ $t("home.selectEngineFirst") }}</div>
    ...
    <pathSelect v-if="editData" :openlist-id="editData.openlistId" ref="pathSelectRef" @submit="submitPath" />
  </div>
</template>
